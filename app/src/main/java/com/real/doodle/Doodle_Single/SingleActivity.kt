package com.real.doodle.Doodle_Single
...
//                  받아온 인덱스 값 저장
        ...
        getSingle(idx!!)
        single_back.setOnClickListener { finish() }
        //        닉네임 눌렀을 때 그사람 피드로 감
...
//        담아감
       single_scrap.setOnClickListener {
           if(!single_nickname.text.equals(CommonData.loginData!!.profile.nickname)){
        ...
        single.enqueue(object : Callback<SingleResponse> {
            override fun onResponse(call: Call<SingleResponse>?, response: Response<SingleResponse>?) {
                if (response!!.isSuccessful) {
                    ...
                    user_idx=response.body().result.user_idx
