    ...
    override fun onBindViewHolder(holder: OtherViewHolder?, position: Int) {
       var holder:OtherViewHolder=holder as OtherViewHolder
        if(otherList!![position].image==null){
            requestManager.load(R.drawable.mytext2).placeholder(R.mipmap.ic_launcher).into(holder!!.OtherImage)
        }
        else{
            requestManager.load(otherList[position].image).placeholder(R.mipmap.ic_launcher).into(holder!!.OtherImage)
        }
